...
describe('attemptTimeout', () => {
  ...
  it('実測（71 件で 151.8 秒）に、最後の回が耐えられる', () => {
    // 2026-09-15 の実測。想定日程つき 71 件が 212 件の予定に展開され、
    // 応答 55,783 文字・151.8 秒かかった。
    // 2 回試す場合でも、最後の回にこれを超える時間が渡ること
    ...
    expect(last).toBeGreaterThan(152_000)
  })
...
describe('持ち時間の値', () => {
  it('画面側の上限（300 秒）より短い', () => {
    // 超えると打ち切られ、日本語のエラーを返す前に通信ごと切れる
    expect(AI_TOTAL_BUDGET_MS).toBeLessThan(300_000)
  })

  it('実測の 151.8 秒に、1 回でも 2 回でも耐えられる', () => {
    // 1 回しか試さない場合は全部を使える
    expect(AI_TOTAL_BUDGET_MS).toBeGreaterThan(152_000)
  })
