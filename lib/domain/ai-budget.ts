 ...
 * 画面側の上限（当時 `maxDuration = 120` 秒）を超えていた。
 *
 ...
 *
 * 2026-09-15 の実測で、**100 秒では足りないことが分かった。**
 * 想定日程つきのタスク 71 件は 212 件の予定に展開され、
 * 応答 55,783 文字・**151.8 秒**かかる。
 * 想定日程を機能に加えたことで、出力量が 2.8 倍に増えていた。
 *
 * そこで画面側の上限を 300 秒へ広げ、ここも 270 秒へ広げる。
 * 実測の 151.8 秒に対し、2 回試す場合でも最後の回に 180 秒が渡る。
 */
export const AI_TOTAL_BUDGET_MS = 270_000
