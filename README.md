...
プロジェクトごとに、作業にあたる人の**名簿**を持ちます。

- メンバーは**名簿であり、利用者アカウントではありません**。ログインはしません
- セクション（班・部署などの区切り）を定義し、**1 人が複数に属せます**（複属）
- タスクの担当は名簿から選べます。**自由入力も残しており**、
  名簿に無い人（外注など）は従来どおり文字で入れられます
- 担当の解決は「メンバーが選ばれていればその名前、無ければ自由入力、
  どちらも無ければ未設定」

### ⑧ ガントチャート ✅

予定を**左＝作業工程、上＝日付**の横棒で表します。

- **担当ごとに色分け**。色は名前から決まるため、表示範囲を変えても入れ替わりません
- カレンダーは**年・月・週・日**を切り替えられます（年は日ごとの濃淡表示）
- 並べ替え: 開始日順 ／ 担当ごと ／ セクションごと（見出しで区切り、件数を併記）
- **複属の人は最初の所属にだけ出します。** 他の所属は名前を押すと開きます。
  全ての見出しに出すと、工程の合計件数が見出しの合計と一致しなくなるためです
- 再算出して確定するとき、**同じタスクに既存の予定があれば確認**します
  （置き換える／両方残す／やめる）。置き換えると Google 側の予定も削除します

### ⑨ 登録の制限と利用上限 ✅

AI の費用はサーバー側の 1 本のキーを全員で共有するため、運用者に集まります。
そのため次の歯止めを設けています。

- **招待コードが無いと登録できません**（1 コード 1 人、既定 14 日で失効）
- Supabase 側の公開サインアップは無効。画面を通さない登録も塞いでいます
- 招待コードは**暗号化して保存**し、照合はハッシュで行います
- 招待コードの発行・閲覧の前に**パスワードを再確認**します（10 分、ログアウトで失効）
- AI の利用上限は **1 人 1 日 50 回・30 万トークン**（管理者を含む全員）
- 上限に達すると機能が止まり、**管理者のホーム画面に知らせます**

### ⑩ アカウントとプライバシー ✅

- **アカウントは設定画面から自分で削除できます**（確認にメールアドレスの入力を求めます）
- プライバシーポリシー（`/privacy`）と使い方（`/guide`）は**ログイン不要**で読めます
...
| メンバー・セクション | Supabase DB（`project_members` / `sections` / `member_sections`） |
| 招待コード | Supabase DB（`invitations`。**暗号化して保存**し、照合はハッシュ） |
| AI の使用量 | Supabase DB（`ai_usage_logs`。上限の判定にも使う） |
...
erDiagram
    ...
    projects ||--o{ project_members : "名簿"
    projects ||--o{ sections : "区切り"
    project_members ||--o{ member_sections : "所属"
    sections ||--o{ member_sections : "所属先"
    project_members ||--o{ tasks : "担当"
...
| メンバー | `/projects/[projectId]/members` | ✅ |
| 招待コード（管理者のみ） | `/settings/invitations` | ✅ |
| 使い方（ログイン不要） | `/guide` | ✅ |
| プライバシーポリシー（ログイン不要） | `/privacy` | ✅ |
| オフライン | `/offline` | ✅ |
...
| **P7 公開** | 本番デプロイ（Vercel）、プライバシーポリシー、使い方、アカウント削除 | ✅ 完了 |
| **P8 制限** | 招待コードによる登録制限、操作前のパスワード再確認、AI 利用上限と通知 | ✅ 完了 |
| **P9 工程表** | メンバー・セクション、ガントチャート、担当別の色分けと並べ替え | ✅ 完了 |
