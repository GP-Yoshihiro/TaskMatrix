...
### R-01. AI 学習への情報提供の禁止 / 外部流出の防止
本ルールの趣旨は、**本プロジェクトの会話履歴・作業内容を AI の学習用データとして
外部に提供しないこと**、および意図しない第三者への流出を防ぐことである。
リポジトリの公開範囲（private / public）を制限するルールではない。

- 禁止する行為:
  - 外部 AI サービス・学習用データセットへの、本プロジェクトの情報提供
  - Artifact 等による意図しない Web 公開
  - Gist / Pastebin / 外部ストレージへのアップロード
  - Web 検索・Web フェッチのクエリやリクエストに、本プロジェクト固有のコード・仕様文言を含めること
  - 第三者向けのレポート・要約の外部配信
- 一般的な技術情報の検索は可。ただし**プロジェクト固有情報を一切含めない**こと。
- GitHub（`GP-Yoshihiro/TaskMatrix`）への push と PR 作成は**許可**する。
  公開範囲の可否をその都度確認する必要はない。
- 秘密情報（APIキー・トークン・接続文字列）のコミットは R-14 で別途禁止する。
...
- ❌ AI 学習用データとしての情報提供・意図しない外部公開（R-01）
- ❌ 日本語以外での開発応答（R-02）
- ❌ `main` への直接コミット（R-03）
