...
- **例外**: 機能の実現に外部サービスへの送信が不可欠な場合は、R-21 の手続きにより実施できる。
...
### R-04. 確認の頻度と PR 運用体制
- コミットの内容確認をユーザーに求めるのは**必要なときのみ**でよい。
- 判断に迷う変更・破壊的変更・仕様解釈が分かれる箇所は必ず確認する。

**PR 運用体制**（2026-08-30 合意）

| 工程 | 担当 |
|------|------|
| ブランチ作成・実装・検証 | Claude |
| **PR の作成**（タイトル・本文・ベース指定） | **Claude** |
| **PR 内容の確認・承認** | **ユーザー** |
| マージ | 承認後に Claude が実施 |

- 機能ブランチの作業が完了し、検証がすべてグリーンになったら、
  Claude が `gh pr create` で PR を作成し、URL をユーザーに提示する。
- ユーザーの承認を得るまでマージしない。
- PR 本文には必ず次を含める。
  1. 概要と対応する設計書・計画のパス
  2. 実装内容の一覧
  3. 検証結果（lint / typecheck / test / build と実動作確認）
  4. 発見・修正した不具合があればその内容
  5. 本フェーズで実装していないもの
  6. レビュー時の注意（マイグレーション適用・環境変数など）
- `gh` CLI の認証はユーザーが行う。Claude は認証操作（トークン・パスワード入力）を行わない。
...
R-01 は情報の外部流出を原則禁止するが、**機能の実現に外部サービスへのデータ送信が
不可欠な場合**があり、これを一律に禁じるとアプリケーションが成立しない。
本規則はその例外を定める。
...
| ケース | 送信先 | 送信する情報 |
|--------|--------|--------------|
| AI タスク抽出・改善提案（P2） | Google Gemini API | プロジェクト内のドキュメント本文 |
| RAG チャット（P4） | Google Gemini API | 検索対象ファイルの本文・埋め込み |
| カレンダー同期（P3） | Google Calendar API | タスク名・日時・算出理由 |
| デプロイ | Vercel / GitHub | ソースコード・ビルド成果物 |
| バックエンド処理 | Supabase | ユーザーが入力・アップロードした全データ |

#### 実施の条件（すべて満たすこと）

1. **必要性**: その送信なしには機能が成立しないこと。代替手段がないこと
2. **事前提示**: Claude は実施前に次を明示すること
   - 何を（送信するデータの種類と範囲）
   - どこへ（送信先のサービス名とエンドポイント）
   - なぜ（実現する機能）
   - どうなるか（送信先での保持期間・学習利用の有無が判明していればそれも）
3. **明示的承認**: ユーザーが承認すること。承認は**案件ごと**に取る
4. **記録**: 承認内容と実施日を `docs/worklog/` に残すこと

#### 承認の有効範囲

- 一度の承認は、**承認された対象・送信先・目的の範囲内でのみ**有効とする
- 同一の機能で繰り返し発生する送信（例: Gemini API へのタスク抽出リクエスト）は、
  その機能に対する承認をもって継続的に有効とする
- **別の送信先・別の目的への流用は不可**。改めて承認を取る

#### 例外の対象外（承認があっても実施しない）

- **AI の学習用データセットへの提供を目的とした送信**
  （R-01 の中核趣旨であり、機能実現のための API 利用とは性質が異なる）
- 秘密情報（サーバー専用キー・トークン）そのものの送信・公開
  ただしサービスへの認証情報としての正当な使用は除く

#### 送信を伴う機能を実装する際の遵守事項

- 送信するデータは**目的に必要な最小限**に絞る
- 外部 API のキーは必ずサーバー側で保持し、クライアントへ露出させない（R-14）
- 送信内容をログに平文で残さない
- ユーザーがどのデータが外部に送信されるかを画面上で把握できるようにする
...
- ❌ AI 学習用データとしての情報提供・意図しない外部公開（R-01）
  - ⚠️ 機能実現に不可欠な外部送信は R-21 の承認手続きにより実施可
- ❌ 日本語以外での開発応答（R-02）
- ❌ `main` への直接コミット（R-03）
